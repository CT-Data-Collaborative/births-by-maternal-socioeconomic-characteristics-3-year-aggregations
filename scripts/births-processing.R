...
for (i in 1:3) { #iterate through parts of this dataset.
  ...
  if ("Mother's Age" %in% names(dataset)) {
    ...
    agg.1519 <- agg.1519 %>%
      group_by(Town, FIPS, Aggregation, Year, `Variable`, `Birth Weight`, `Gestational Age`, `Measure Type`, `Mother's Race/Ethnicity`) %>% 
      mutate(Value = sum(Value))
    agg.1519$"Mother's Age" <- "15 to 19 years"
    setDT(agg.1519)
  ...
  dataset$Value[which(dataset$`Measure Type` == "Number" & dataset$Value > 0 & dataset$Value < 15)] <- -9999
  ...
  dataset$Value[which(dataset$`Measure Type` == "Percent" & dataset$calc_num > 0 & dataset$calc_num < 15)] <- -9999
  dataset$calc_num <- NULL
  ...
  if ("Mother's Age" %in% names(dataset)) {
    dataset <- select(dataset, Aggregation, Town, FIPS, Year, `Birth Weight`, `Gestational Age`, 
                      `Mother's Age`, `Mother's Race/Ethnicity`, `Measure Type`, Variable, Value)
    #setcolorder(dataset, c(1:4,7,8,10:ncol(dataset),9,5,6))
  } else if ("Mother's Education" %in% names(dataset)) {
    dataset <- select(dataset, Aggregation, Town, FIPS, Year, `Birth Weight`, `Gestational Age`, 
                      `Mother's Education`, `Mother's Marital Status`, `Measure Type`, Variable, Value)
  } else {
    dataset <- select(dataset, Aggregation, Town, FIPS, Year, `Birth Weight`, `Gestational Age`, 
                      `Initiation of Prenatal Care`, `Smoking During Pregnancy`, `Measure Type`, Variable, Value)
  }
